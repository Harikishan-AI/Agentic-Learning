...
# search agent Binded with tool
search_agent_tool = AgentTool(search_agent)


# Now the search agent tool can be used with custom tool 
# If we directly feed the builtInTool with the custom tool then it would throw an error [as per ADK doc]
root_agent = Agent(
    model='gemini-2.0-flash-001',
    name='root_agent',
